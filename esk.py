...
def updateReviewers():
  ...
  for r in repos:
    chngreq = requests.get("https://review.lineageos.org/changes/?q=status:merged+project:"+r['full_name']+"&n=20&o=MESSAGES").content
    ...
    if len(r['maintainers']) > 0:
      ...
      for row in rows:
        ...
        if r['full_name'] == row[0]:
          ...
          row[2] = " ".join(list(mset)).replace("@", "OBFusCAAT")
      if not edited:
        rows.append([r['full_name'], "", " ".join(r['maintainers']).replace("@", "OBFusCAAT")])
  with open(absPathTo("repos_extras.txt"), "w") as f:
    writer = csv.writer(f, delimiter=" ", quoting=csv.QUOTE_MINIMAL)
    for row in rows:
      writer.writerow(row)
